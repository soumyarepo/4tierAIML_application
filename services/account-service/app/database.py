"""Database configuration for the Account Service.

Creates the async SQLAlchemy engine and session factory for the bank_accounts
database.
"""
...
def get_engine() -> AsyncEngine:
    ...
    if _engine is None:
        raise RuntimeError("Account service engine not initialized. Call init_db() first.")
    return _engine
...
def get_session_factory() -> async_sessionmaker[AsyncSession]:
    ...
    if _session_factory is None:
        raise RuntimeError(
            "Account service session factory not initialized. Call init_db() first."
        )
...
async def init_db(echo: bool = False) -> None:
    ...
    database_url = settings.get_bank_accounts_db_url()
